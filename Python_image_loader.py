...
#load metadata file with Hashes if file not exist, hash all files in destination first
class Python_image_loader:
    def __init__(self, sourceDir, destDir):
        ...
        self.subscription_key = "AZURE API KEY GOES HERE"
                        ...
                        with open(path.join(root, name), 'rb') as f:
                            ...
                            while True:
                                ...
                                if not data:
                                    break
                                sha1.update(data)
                            line = DataFrame([{"SHA256":sha1.hexdigest(), "Path":path.join(root, name), "Azure_Processed":0}])
                            hash_dict = concat([hash_dict, line], axis=0, ignore_index=True)
    ...
    def azureVisionUpdate(self, image_path):
        ...
        if len(main_category) > 0:
            prefix = max(analysis["categories"], key=lambda x: x['score'])['name']
        else:
            prefix = 'NoCat'
    ...
    def findPics(self):
        ...
        print('[+] Examining new files and checking for viable backgrounds (jpg, 1920x1080 only)')
        ...
        if newHashes.shape[0] > 0:
            ...
            for index, row in newHashes.iterrows():
                copyfile(row['Path_x'], path.join(self.destinationDir, path.basename(row['Path_x']+'.jpg')))
                line = DataFrame([{"SHA256":row['SHA256'], "Path":path.join(self.destinationDir, path.basename(row['Path_x']+'.jpg')), "Azure_Processed":0}])
                self._copyLog = concat([self._copyLog, line], axis=0, ignore_index=True)
        ...
        for index, row in self._copyLog.loc[self._copyLog['Azure_Processed'] == 0].iterrows():
            if self.__unprocessedFileName(path.basename(row['Path'])):
                print('[+] Starting Azure image tagging for {} ...'.format(path.basename(row['Path'])))
                ...
                try:
                    ...
                    print('[+] Azure image tagging finished, new name: {}'.format(newFileName))
                except OSError:
                    print('[ERROR] Image {} not updated'.format(path.basename(row['Path'])))
        self._copyLog.to_csv(self._copyLogFile, quoting=1)
...
def main():
    if len(argv) < 2:
        ...
        sourceDir = getenv('LOCALAPPDATA')+"\\Packages\\Microsoft.Windows.ContentDeliveryManager_cw5n1h2txyewy\\LocalState\\Assets\\"
        ...
        if len(argv) == 2:
            ...
            print('[ERROR] wrong arguments given, format is: PythonImageLoader.py Source Destination')
            exit()
